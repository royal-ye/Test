...
require 'lib/myUtils' --myUtils_local is the local loss&global_loss and the opt.local_loss need to be changed
...
opt = {
   ...
   name = 'En_De_GAN_1W',              -- name of the experiment, should generally be passed on the command line
...
local idx_mask = nil --gai
idx_mask = {input_nc*2+1, input_nc*2+1}
...
if opt.continue_train == 1 then
   ...
   netG = util.load(paths.concat(opt.checkpoints_dir, opt.name, '50_net_G.t7'), opt)
   print('loading previously trained netD...')
   netD = util.load(paths.concat(opt.checkpoints_dir, opt.name, '50_net_D.t7'), opt)
	if opt.local_loss == 1 then
	netD_local = util.load(paths.concat(opt.checkpoints_dir, opt.name, 'latest_net_D_local.t7'), opt)
	end
else
...
if opt.local_loss == 1 then
local real_AB_local = torch.Tensor(opt.batchSize, input_nc, opt.fineSize, opt.fineSize)
end
...
local errD, errG, errL1 ,errD_local,errG_local= 0, 0, 0, 0, 0
...
if opt.gpu > 0 then
   ...
   if opt.cudnn==1 then
	...
	if opt.local_loss == 1 then
	netD_local = util.cudnn(netD_local)
	netD_local:cuda()
	end
   end
...
if opt.local_loss == 1 then
local parametersD_local, gradParametersD_local = netD:getParameters()
end
...
function createRealFake()
	...
	mask_global:copy(real_data[{ {}, idx_mask, {}, {} }])
...
local fDx_local = function(x)
    netD_local:apply(function(m) if torch.type(m):find('Convolution') then m.bias:zero() end end)
    netG:apply(function(m) if torch.type(m):find('Convolution') then m.bias:zero() end end)
    
    gradParametersD_local:zero()
    
    --Train Real
	--print(torch.type(real_AB_local))
    local output = netD_local:forward(real_AB_local)
	...
	local errD_real = criterion:forward(output, label)
    local df_do = criterion:backward(output, label)
    netD_local:backward(real_AB_local, df_do)

    --Train Fake
    local output = netD_local:forward(fake_AB_local)
	
    label:fill(fake_label)
    local errD_fake = criterion:forward(output, label)
    local df_do = criterion:backward(output, label)	
    netD_local:backward(fake_AB_local, df_do)	
    errD_local = (errD_real + errD_fake)/2	
    return errD_local, gradParametersD_local
end
...
local fGx = function(x)
    ...
    local df_dg_tmp = torch.zeros(fake_B:size())
    if opt.gpu>0 then 
    	df_dg = df_dg:cuda();
        --df_dg_tmp = df_dg_tmp:cuda();
    end
    ...
    local df_dg_local = torch.zeros(fake_AB_local:size())
    if opt.gpu>0 then 
    	df_dg_local = df_dg_local:cuda();
    end
    local output_local = netD_local.output
    local label_local = torch.FloatTensor(output_local:size()):fill(real_label)
    if opt.gpu>0 then 
       	label_local = label_local:cuda();
    end
    errG_local = criterion:forward(output_local, label_local)
    local df_do_local = criterion:backward(output_local, label_local)
    df_dg_local = netD_local:updateGradInput(fake_AB_local, df_do_local):narrow(2,fake_AB_local:size(2)-output_nc+1, output_nc)
	...
	if opt.local_loss == 1 then
    df_dg_tmp[{{},{},{left,left+127},{top,top+127}}] = df_dg_local:clone() 
	end

    netG:backward(real_A, df_dg:mul(opt.beta) + df_do_AE:mul(opt.lambda))--+ df_dg_tmp:mul(opt.beta))
...
for epoch = 1, opt.niter do
    ...
    for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
		...
		if opt.local_loss ==1 then 
		print("local")
		optim.adam(fDx_local, parametersD_local, optimStateD_local) 
		end
        ...
        if counter % opt.display_freq == 0 and opt.display then
            ...
            if opt.preprocess == 'colorization' then 
                local real_A_s = util.scaleBatch(real_A:float(),200,200)
                local fake_B_s = util.scaleBatch(fake_B:float(),200,200)
                local real_B_s = util.scaleBatch(real_B:float(),200,200)
        ...
        if counter % opt.save_latest_freq == 0 then
			...
			if opt.local_loss == 1 then
			torch.save(paths.concat(opt.checkpoints_dir, opt.name, 'latest_net_D_local.t7'), netD_local:clearState())
			end
        end
	...
	if opt.local_loss == 1 then
	parametersD_local, gradParametersD_local = nil, nil
	end
    ...
    if epoch % opt.save_epoch_freq == 0 then
		...
		if opt.local_loss == 1 then
		torch.save(paths.concat(opt.checkpoints_dir, opt.name, epoch .. '_net_D_local.t7'), netD_local:clearState())
		end
    end
    ...
    --parametersD_local, gradParametersD_local = netD:getParameters()
end
