...
opt = {
   ...
   name = 'end2end_64_6_16',              -- name of the experiment, should generally be passed on the command line
   which_direction = 'AtoB',    -- AtoB or BtoA
   phase = 'train_1W',             -- train, val, test, etc
...
function defineG_unet_swap_mask_end2end(input_nc,output_nc,ngf, idx, conv_layers, guideLayer64, swapLayer64)
...
-- encoder-decoder part
    ...
    -- input is (ngf * 2) x 64 x 64
    local d6_d_gd = {d6_d, maskModel} - nn.JoinTable(2) - nn.ReLU(true) - guideLayer64  -- guide layer fork  , as maskModel is positive, so it is fine.
...
function defineG(input_nc, output_nc, ngf)
    ...
    elseif opt.which_model_netG == "unet_swap_mask_end2end" then netG = defineG_unet_swap_mask_end2end(input_nc, output_nc, ngf, 1, conv_layers, guideLayer64, swapLayer64)
...
--********** reassgin the correct maskModel weigth**************
    netG.modules[74].weight:fill(1/16)
    netG.modules[75].weight:fill(1/16)
    return netG
end
