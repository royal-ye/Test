...
require 'image'
paths.dofile('datasetC.lua')
-- This file contains the data-loading logic and details.
-- It is run by each data-loader thread.
------------------------------------------
-------- COMMON CACHES and PATHS
-- Check for existence of opt.data
DatasetName=os.getenv('DATA_ROOT') or opt.DATA_ROOT
print(DatasetName)
opt.data = paths.concat(DatasetName, opt.phase)
...
local loadSize   = {input_nc/2, opt.loadSize}
local sampleSize = {input_nc/2, opt.fineSize}

local preprocessAandBC = function(imA, imB,imC)
  imA = image.scale(imA, loadSize[2], loadSize[2])
  imB = image.scale(imB, loadSize[2], loadSize[2])
  imC = image.scale(imC, loadSize[2], loadSize[2])
...
--     print('process regular')
     ...
     imA, imB,imC = preprocessAandBC(imA, imB,imC)
     temp = torch.cat(imA, imB, 1)
     imAB = torch.cat(temp, imC, 1)
    --print('image C size')
   --print(imAB:size())
   end
