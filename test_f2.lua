-- Make sure using the threshold is consitent with train
...
opt = {
    ...
    name = 'swap64_GAN',                -- name of experiment, selects which model to run, should generally should be passed on command line
    ...
    which_epoch = '20',            -- which epoch to test? set to 'latest' to use latest cached model
    threshold = 6/16,
	test = 1,
}
...
mask_global = create_gMask(pattern, mask_global, MAX_SIZE, opt):squeeze()

--image.save('2.png',mask_global:squeeze())
...
for n=1,math.floor(opt.how_many/opt.batchSize) do
    ...
    -- test U-net with swap in the latter.
    -- netG.forwardnodes contains 'input' node, not consistent with netG.modules.
        netG:forward(input)
        mask_global = mask_global:squeeze()
        for indexNode, node in pairs(netG.forwardnodes) do
            if indexNode == 48 then
                if node.data.module then
                    style_latent2 = node.data.module.output:clone()
                end          
            end
        end
        swap2 = nn.InnerSwap('innerSwap2', mask_global, 2, style_latent2, 1, opt.threshold, 1)
